...
Run `aep skills` to discover procedures, then read `aep skills show <name>` and selected references. The working agent chooses skills and work order. This source repository can read its bundled guidance with `cargo run -p aep-cli -- skills` before installation.
