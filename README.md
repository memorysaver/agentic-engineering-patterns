...
`aep eval` is a neutral view of how a project is run. `aep eval snapshot` collects structural facts (records, verification policy, delivery receipts, Git state, legacy residue, CI and secret scanning) into a run under `~/.aep/eval/<project>/<run>/` and `aep eval report` applies fixed rules that name the engineering practice behind each finding. Nothing is written into the project. Under Herdr, you open a pane for the observer, start an agent there and tell it which agent to watch; `aep eval watch` lists the candidates, the agent confirms the target with you, and `aep eval watch --target <pane>` turns it into the observer following `aep --skill eval`: `aep eval tick` paces it (at least ten minutes between looks) and reports what changed; only then does it read the transcript, compare claims with records and Git, and store an observation with `aep eval record`. The observer never sends input to the working agent and never judges product decisions or feature value.
