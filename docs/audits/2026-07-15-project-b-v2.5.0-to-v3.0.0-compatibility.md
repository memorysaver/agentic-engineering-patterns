# Project B downstream compatibility audit: AEP v2.5.0 → v3.0.0
...
**Downstream baseline:** project-b `main` at `12a3bef942c92b9276fa2a118356da8cbffbd921`  
**Downstream path inspected:** `/Users/memorysaver/Documents/github/project-b`

## Executive verdict

**Project B is conditionally compatible with AEP v3.0.0.** Its product-context data, split-file layout,
...
| Area                                | Result                          | Meaning                                                                                                                  |
| ----------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Product-context structure and YAML  | **GREEN**                       | No schema migration required; v3 detects project-b as Split mode.                                                          |
...
merge + wrap and the autopilot workspace map is empty.** Do not mutate the current project-b checkout in
place.
...
1. Are the bytes installed in project-b actually the stated v2.5.0 baseline?
2. Which v2.5.0 → v3.0.0 differences only change context loading, and which intentionally change agent behavior?
3. Can v3 read project-b's real product context, calibration, object-model, story graph, layer gates, and runtime state?
...
- byte comparison of every installed project-b AEP `SKILL.md` against tag `v2.5.0`;
...
- v3 `aep-scaffold/scripts/audit.sh` against the real project-b checkout (read-only);
...
purpose-built parsers. No files in the real project-b checkout were changed.

## Baseline integrity

### What project-b is actually running

- Project B's `AGENTS.md` states AEP `v2.5.0`.
- The v2.5.0 marketplace contains **21** AEP skills.
- Project B has those same 21 physical AEP skill directories in `.agents/skills/`; every installed
  `SKILL.md` byte-matches its v2.5.0 source.
- `.claude/skills/aep-*` correctly points to the real `.agents/skills/aep-*` copies.
- Project B's `skills-lock.json` contains **22** AEP entries because it also contains a stale
...
- the 22 behavior cases are review dry-runs, not repeated runtime distributions over project-b prompts;
...
## Project B product-context compatibility
...
- the canonical product-context template uses structured descriptions, numeric-or-null business value,
  `in_review → completed` plus `deferred`, and the richer layer-gate evidence shape that project-b uses.
...
contract drift rather than rewriting project-b's working data to the stale protocol prose.
...
The rehearsal cloned project-b's tracked HEAD, leaving the real repo and its untracked files untouched.
...
- the project-b project test/check suite is unchanged and green;
...
| Validate project-b product context without implementing | `aep-validate`    |
...
Track the known protocol-spec false positives separately rather than “fixing” canonical project-b fields to
...
No evidence requires a product-context migration or blocks project-b from adopting AEP v3.0.0. The data
...
rewriting project-b's accumulated product context.
