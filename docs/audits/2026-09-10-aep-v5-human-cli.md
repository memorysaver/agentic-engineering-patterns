...
Each real project ran nine successful commands: help, root skill, project verification skill, doctor, text status, JSON status, check, migration verification and dry-run init. Before/after hashes of all Git-listed tracked and nonignored untracked files matched: Project B 4,805; Project A 2,631; Project C 2,987. Status summaries were bounded and preserved readiness constraints. The machine evidence records exact commands, exit codes and line counts; full transcripts remain under `~/.local/share/aep/trials/2026-09-10-human-cli-live/`.

Active AGENTS, rules and host adapters in all three migration branches now use `aep --skill`. Independent review confirmed that the 28 edits only replace command names; all four edited TOML files parse and the Project C skill frontmatter is unchanged. Historical imported sources were retained. Project A and Project C `bun run check` passed, retaining their existing exact-content formatting debt boundaries (86 and 45 files). Project C operational-truth validation passed. These interface checks do not establish product acceptance, exercise external providers or change the paused Project C worktree.
