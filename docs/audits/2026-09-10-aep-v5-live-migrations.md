...
Project A and Project C are clean. Project B retains exactly its four original unrelated untracked files; its preexisting convention edit was committed separately without changing its bytes. Project C's unfinished OBS feature worktree remains at its original HEAD with all 13 dirty-file hashes unchanged. Its old autopilot state is paused and backed up; native OBS-001 remains blocked for an explicit resume.

Independent reviews closed the native scope/provenance, sensitive-prefix, host-role and native-reader findings. Final root review additionally compared all 107 Project A records across its last scope correction: only paths changed on 100 records. No original lifecycle or dependency field changed. No migration branch was pushed or merged by this operation.
...
The standalone candidate outside the source tree ran nine commands in each original repository: `doctor`, `skills`, `skills show validate`, `status`, `query`, selected-story `context` and `timeline`, `check`, and `migrate verify`. All **27 commands passed**. Selected stories were Project B `L41-006b`, Project A `A-107`, and Project C `OBS-001`, including retained blocked/imported states.

Before/after hashes of tracked and nonignored untracked regular files were identical: 4,805 files in Project B, 2,631 in Project A, 2,987 in Project C. These inspection commands did not mutate project files or advance work. Exact revisions, status distributions and command outcomes are in [the machine-readable trial evidence](evidence/2026-09-10-v5-live-cli-trial.json).
...
Earlier in these same live migrations, Project B's local Worker SQLite suite passed 31 tests; Project A passed its Rust help smoke suite (10 tests), repository/type checks and retained-evidence checks; Project C passed 122 governance/reader/documentation/formatter tests and 17 final focused tests, actual operational/documentation truth, provider-free preflight and secret scan. Narrow scanner exceptions retained negative controls. Existing formatter debts remain visible and bound to exact pre-migration bytes (Project A 86; Project C 45).
