...
| Downstream migration          | Project B exposed binary evidence decoding; Project A exposed overlapping OpenSpec/story IDs, lost deferred holds and prospective configuration validation. Fixes include source-scoped alias review and fresh/existing configuration reproductions; all bounded review findings are closed |
...
Linux and both macOS targets passed on `808fa3e`; the additional downstream fixes require checks on the updated [PR #35](https://github.com/memorysaver/agentic-engineering-patterns/pull/35) head. The owner selected Project B, Project A and Project C. Their [isolated pilot observations](2026-09-10-aep-v5-downstream-pilots.md) distinguish native context conversion, actual project verification, and remaining executable consumer or specification gaps. The original three projects remain untouched.
