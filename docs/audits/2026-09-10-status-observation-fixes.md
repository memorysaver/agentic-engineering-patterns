# Project A 狀態觀察後的已確認錯誤修正

2026-09-10。使用者在[原始觀察](../lessons/2026-09-10-project-a-v5-status-observation.md)完成後授權「先修正已知的錯誤」。本次處理 OBS-01 的 CLI reference 契約，以及 OBS-04 的 Project A adoption 相容性；原始觀察保留當時狀態。
...
## Project A adoption 與精簡 AGENTS 的相容性

透過原 Project A Herdr agent 執行，story `A-108`；implementation worktree 為 `Project A-adoption-fix`。修正 commit：`185ac2eea70c9e1f8516f8df403ecfedc472241e`，review 後修補 commit：`c2e390ac0ab4afba5325d2d299c672e0e46c17cc`。

產品修正在 `aep/A-108-attempt-18d3fb19e1b60eaa-eb700-0`；共享帳本與驗證紀錄在 `fix/adoption-guidance-context` 的 `1d775db`。Project A 的 `project-ledger/changes/adoption-indexed-guidance/verification.md` 索引保留的測試、獨立 review、實際安裝前後與回滾證據。Native 命令使用共享根目錄 `Project A`，worktree 內的 story 是基底快照。

Adoption 檢查保留既有完整 AGENTS 內文的支援，也辨識明確的 `AGENTS.md → project-rules/README.md → project-a-memory.md` 路徑。三份文件均保持原樣。檢查會讀取可達的規則內容；只有檔案存在、範例文字或指向專案外的 symlink 並不足以通過。已有索引但路徑缺漏時，install-memory 保留文件並要求修復索引，不將完整政策重新附加到 AGENTS。
...
保留既有 `experimental-local-embeddings` 功能，以 Rust 1.95.0 建置 release，更新 `/home/memorysaver/.cargo/bin/project-a`。未 merge 或公開發布。
...
- 備份、manifest、before/after audit 與 deep status：`~/.local/state/project-a/binary-backups/20260910-adoption-wrge3phk/`。
- 回滾工具：同目錄 `rollback.py`；dry validation 通過，沒有實際回滾本次修正。

實際 Project A repository：`agents_trigger_present` 由 false 轉為 true，deep status 的 `hard_failures` 從僅包含 `adoption_missing_agents_trigger` 變為空陣列，`quality.overall_pass` 由 false 轉為 true。原有 claims sidecar byte limit 與 `duplicate-collapse=limit_absent` 警告仍存在。本輪證據並不表示歷史 blocker 或 recall 上限問題已修復。
