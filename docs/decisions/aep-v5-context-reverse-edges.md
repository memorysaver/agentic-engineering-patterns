...
Date: 2026-09-17
Evidence: [project-b basic user access delivery](../lessons/2026-09-15-project-b-basic-user-access-delivery.md) (sections "Against the open v5 items", "Rule audit", turns six through ten)
...
Measured on project-b at `aep 5.0.0-preview.1` build `3fae0e42…` after two days of native deliveries:
...
- Downstream: after installing the rebuilt binary as the shared `aep`, rerun on project-b `aep context basic-user-access --json` (members present), `aep context BASIC-001 --json` (lesson and release present), and `aep check` (PASS with warnings listed, exit 0). Record the before/after digests and outputs in the trial bundle.
