...
**C-P4 is deferred past v4.0.0, deliberately.** It is the one phase whose gate cannot be satisfied by reasoning about the change: R7 requires a fresh **independent triggering run** — a probe agent given only the installed names and descriptions, whose selections are recorded in `evals/skill-routing-observations.json` and bound to a sha256 of those descriptions. That binding exists precisely so a description diet cannot silently un-wire a skill, and re-recording it without actually running the probes would defeat the only mechanism that makes the diet safe. Everything else in this document ships in v4.0.0; C-P4 ships when the run has been made.
