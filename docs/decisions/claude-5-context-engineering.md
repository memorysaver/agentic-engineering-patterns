...
| ------------------------------------------- | -------------------------------------------- | ----------------------------------------- |
| SKILL.md files / lines                      | 23 / 5,197 (max 398)                         | 24 / 5,257 — the original 23 at 5,213 (+16, near-flat); the 24th is `/aep-easy-explain` (44 lines, user-typed-only) |
...
| Model-invocable skills                      | 23 of 23                                     | 23 of 24 — the original 23 stay advertised; `/aep-easy-explain` is user-typed-only by design. See the C3 amendment |
...
| **C-P4 — description budget (C3)** | Diet to 3,369 characters, re-recorded routing evals, CI cap to 3,400. The proposal to de-advertise existing skills and add a router was dropped per the C3 amendment (a new skill may still be born user-typed-only)                                  | **Routing metadata** — the risk phase |
...
**Not established.** Any end-to-end efficiency delta, on any host. `evals/skill-behavior-parity.json` is a reviewed dry-run inventory — CI checks its shape and that the disclosed references exist; it does not execute behavior. No cross-model routing probe has run. Installed skill bytes grew 1,702,331 → 1,786,316 (+83,985, **+4.93%** — typed artifacts and their materialized copies), and SKILL.md went 23 files / 5,197 lines → 24 / 5,257 (+1.16%): the original 23 sit at 5,213 (+16, near-flat), and the 24th (`/aep-easy-explain`, 44 lines) is user-typed-only. Total bytes and always-loaded context are different axes; only the second was dieted and measured. The representative-task benchmark (v3.3.1 vs v4.0.0, Claude and Codex hosts) is post-merge work on its own branch.
