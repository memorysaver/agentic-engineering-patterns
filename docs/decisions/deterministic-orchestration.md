...
> **Status:** Accepted — implemented in **v2.7.0** (PR #22, shared with build-convergence-pipeline.md). This document records the design precisely; the implementation was reviewed against it. It changes how AEP works, so it lives in `decisions/` per the [docs routing guide](../README.md).
