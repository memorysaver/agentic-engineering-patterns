...
Roll out first in a downstream fixture, then one representative existing monorepo, then additional old-project layouts. Name the pilot from current inventory when implementation starts. Keep active stories on their original contract. Retain aliases/readers where active consumers need them, with Git available for older records. The default 5.0 install uses CLI guidance instead of per-runtime skill bundles. Follow the [release convention](../../project-rules/release.md); the overarching Rust CLI design selects AEP 5.0.0 for the full contract change.
