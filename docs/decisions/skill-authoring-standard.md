...
- **R7 — Budgets.** One corpus-wide budget for every SKILL.md: **warn > 400 lines, hard cap 500** (aligned with Anthropic's ~500-line skill guidance). The per-skill targets in the refactor table below are the binding review gate for each implementation PR; "utility/index skills aim for ≤ 150" is review guidance, not a separate CI tier. **Implementation note (v3.0.0):** the post-review corpus is 4,873 lines vs the table's ~4,455 — several skills (worst: watch +37%, launch +34%, autopilot +28%) settled above their table estimate where R9 behavior preservation won over the size target. The deviations are accepted; the enforceable ceiling remains this rule's CI budget, which every file meets (max 398). Descriptions: ≤ 50 words, one trigger per branch, no synonym stacking — and any description whose trigger set shrinks must pass a triggering check (probe prompts or a skill-creator-style eval) before merge, since dropped trigger terms can silently stop a skill firing. **Metadata implementation note (2026-07-14):** descriptions are 652 words / 4,697 characters corpus-wide (max 35 words per skill); CI caps each description at 300 characters and the corpus at 5,000. `evals/skill-routing.json` owns expectations; independent selections and their installed-description digest live in `evals/skill-routing-observations.json`.
- **R8 — CI enforcement** in `skills-check.yml`: (a) line check over `skills/**/SKILL.md` — warn > 400 from P1; the fail threshold (> 500) activates in the P3 PR once the corpus is under budget (activating earlier would fail the not-yet-refactored files); (b) the multi-line `$BASE` resolver block appears in exactly one **SKILL.md** (git-ref) — the check greps **two signatures**: the `aep.integration-branch` literal AND the develop-probe bash (`refs/heads/develop` / `refs/remotes/origin/develop`), since a resolver copy that omits the config override still probes develop and would otherwise evade the check. Both legitimately survive in reference files whose bash must self-resolve at runtime (`executor/references/backends.md` probe, `autopilot/references/tick-protocol.md` nudge prompt, `build/references/harness-artifacts.md` init.sh recovery script), so those are allowlisted and the check must not scan all of `skills/`; (c) existing `skills:check` stays green; (d) `check-skills-package.sh` requires exact marketplace/source coverage, a real 22/22 copied install, installed-link containment, metadata and evidence budgets, official `agentskills` validation of every installed unit, no broken or cross-skill relative links, and non-increasing navigation-debt ratchets (26 reference-to-reference links; 28 unique >100-line references without an early TOC); (e) build/audit/converge fixtures cover marker traversal and destination shapes, command-injection-safe names, AEP drift, exact gitignore entries, one-sided installs, content/mode divergence, parent and whole-directory symlink escapes, category filtering, idempotency, and write-failure propagation.
- **R9 — Behavior parity.** Every restructure PR (P1–P3) attaches parity evidence for each touched skill: at least one recorded scenario dry-run (or `/aep-validate` gen-eval pass) showing the leaned skill executes the same steps and loads its disclosed references when the scenario requires them. Line counts prove size, not behavior; a disclosure behind a weakly-worded pointer passes every other gate while silently regressing the skill — R9 is the gate that catches it. **Evidence note (2026-07-14):** `evals/skill-behavior-parity.json` records 22/22 final-review scenario dry-runs, disclosed references, and observable postconditions; `check-skills-package.sh` requires exact catalog coverage. Front-tier metadata selection is separately recorded in `evals/skill-routing-observations.json` and bound to the installed-description digest.
...
Per-skill targets from the audit. "Lean" = steps kept inline, listed material moved to `references/` or replaced by pointers; no step is removed or reordered. The per-skill moves (what stays, what discloses where, what dedups to which canonical home) are expanded in [plans/2026-07-14-lean-skills-refactor.md](../plans/2026-07-14-lean-skills-refactor.md). **Implementation exception:** `/aep-reflect`'s former Step 5.5 (Workflow Improvement) was folded into Step 2's Process classification branch. Its document/update/propose actions remain, but the step name and position did change; review parity by behavior, not by the original structural claim.

| Skill                 |       Now |     Target | Primary moves                                                                                                                                                                                                                                                       |
| --------------------- | --------: | ---------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| build                 |       955 |       ~325 | Phase 0 artifacts, Phase 5 eval machinery, Phase 6 dogfood theory, Phase 12 merge taxonomy → references; build Phase 0 stays canonical — worktree-onboarding.md removed after review (launch's assembled bootstrap carries the orientation; no remaining consumers) |
...
- [`evals/skill-routing.json`](../../evals/skill-routing.json) — recorded metadata-only routing probes for every skill plus high-confusion boundaries; rerun when names or descriptions change.
