...
- **R8 — CI enforcement** in `skills-check.yml`: (a) line check over `skills/**/SKILL.md` — warn > 400 from P1; the fail threshold (> 500) activates in the P3 PR once the corpus is under budget (activating earlier would fail the not-yet-refactored files); (b) the multi-line `$BASE` resolver block appears in exactly one **SKILL.md** (git-ref) — the `aep.integration-branch` literal legitimately survives in reference files whose bash must self-resolve at runtime (`executor/references/backends.md` probe, `autopilot/references/tick-protocol.md` nudge prompt, `build/references/harness-artifacts.md` init.sh recovery script), so those are allowlisted and the check must not scan all of `skills/`; (c) existing `skills:check` stays green.
