...
- The File-Resolution SPLIT/V1 block repeats in all 8 product-context skills (~70 lines); the YAML-validation-plus-common-fixes prose repeats 4–7× while every one of those skills already carries `references/yaml-guardrails.md`; executor/gen-eval/workflow/design-lens each list their reference filenames **twice** (a prose path block and a Reference Files table).
...
Every rule below is checkable in review; R7–R8 are checkable in CI.
...
- **R2 — Single source of truth.** Each meaning has exactly one canonical home; every other site is a one-line pointer. Canonical-home table:
  ...
  | File-Resolution SPLIT/V1 probe                                                               | one `_shared` reference, pointed at by all product-context skills |
...
- **R7 — Budgets.** SKILL.md ≤ 250 lines (lifecycle skills) / ≤ 150 (utility/index skills); description ≤ 50 words, one trigger per branch, no synonym stacking. Budgets are review gates, not aspirations.
- **R8 — CI enforcement** in `skills-check.yml`: (a) line-budget check over `skills/**/SKILL.md` (warn at budget, fail at 1.5×); (b) the `$BASE` resolver literal (`aep.integration-branch`) appears in exactly one file under `skills/`; (c) existing `skills:check` stays green.
...
| --------------------- | --------: | ---------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| build                 |       955 |       ~325 | Phase 0 artifacts, Phase 5 eval machinery, Phase 6 dogfood theory, Phase 12 merge taxonomy → references; worktree-onboarding.md becomes the single Phase-0 source (SKILL points at it) |
...
| design-lens           |       243 |       ~145 | Design Decisions + earns-its-place table → references/decisions                                                                                                                        |
...
| **P1 — mechanical dedup sweep**              | `$BASE` → git-ref one-liner everywhere; File-Resolution + YAML-guardrails → pointers; delete duplicate reference-path blocks; nudge prompts live only in tick-protocol.md; standardize cross-links to `/aep-x` prose (fixes dual-path bug); CI checks (R8) | No               |
| **P2 — big-four restructure**                | build, autopilot, dispatch, scaffold to target shape (one PR each)                                                                                                                                                                                         | No               |
| **P3 — corpus pass**                         | remaining 18 skills to budget; Design Decisions essays → `docs/decisions/`; negation→positive + no-op pruning; description diet; glossary entries; v2.8.0 bump                                                                                             | No               |
...
- Product-context skills' `references/` and `templates/` are **build-generated from `_shared/`** — every P1/P3 edit to those files goes to `_shared/` first, then `scripts/build-skills.sh`; authoring directly in a consumer dir gets silently wiped on commit.
...
- Downstream consumers see nothing until the v2.8.0 tag is cut and each of the 6 repos re-pins via the skills CLI. P1–P3 must not change any step semantics, so a re-pin is safe by construction; `skills:check` green is the release gate.
...
- **Synonym triggers.** Description trigger lists that restate one branch five ways pay permanent context load for zero extra recall.
