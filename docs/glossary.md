...
> The following terms are defined in the [v2 Improvement Guideline](aep-v2-improvement-guideline.md) and are **not yet implemented**. They describe the planned evolution from single-file `product-context.yaml` to a multi-map document architecture. When v2 ships, these terms will move to their proper sections above.

### Three-Layer Architecture

The core v2 structural change. Replaces the single `product-context.yaml` with three distinct layers: **Human Maps** (product narrative for humans), **Machine Graph** (compiled agent control plane), and Change Packets (OpenSpec, already exists). Separates human sensemaking from machine execution.

See also: **Human Maps**, **Machine Graph**

### Human Maps

The product narrative layer in the **Three-Layer Architecture**. Organized by capability, one set of files per user journey. Contains what humans need: opportunity brief, personas, backbone activities, layers with outcome hypotheses, and story stubs. Does NOT contain dispatch state, agent topology, or cost tracking.
...
The compiled agent control plane (`product/machine-graph.yaml`). Generated by `/compile` from all **Human Maps**. Contains everything agents need: full stories with state machine, dispatch scoring, architecture, topology, layer gates, cost tracking, and learning policy. Written by `/compile` (initial), `/dispatch` (story state), `/wrap` (completion state).

### Capability Map

A per-capability set of files (`frame.yaml` + `map.yaml`) that tells one complete user narrative. Each map covers a single user journey or value stream with its own backbone, layers, and story stubs. Even simple products start with one capability map — the structure supports growth without restructuring.

### /compile

Proposed skill that transforms **Human Maps** into the **Machine Graph**. Uses tiered compilation: Tier 1 (mechanical — copying fields, formula computation, DAG assembly) is deterministic; Tier 2 (AI-assisted — expanding acceptance criteria, inferring interfaces) requires human review.

### /slice

Proposed skill for human-driven release line cutting. Presents backbone activities across capabilities, lets the human decide which layers to target in the current release cycle. Implements Jeff Patton's "release line is a pencil line" philosophy.

### Readiness Score

A per-story spec completeness score (0.0–1.0) computed during `/compile` or `/validate`. Components: acceptance criteria count, interface obligations defined, files affected identified, verification defined, no unresolved open questions. Routes stories: < 0.5 → `/design`, >= 0.7 → `/launch`, between → user decision.

### Outcome Contract

A testable hypothesis attached to each capability frame and layer. Format: "If users can do X, then Y business outcome follows" with a success metric, target, and decision rule (keep_if / otherwise). Evaluated by `/reflect` after layer completion — not automated test results, but product-level learning.

### Learning Policy

A new section in the **Machine Graph** that auto-adjusts dispatch parameters based on execution history. Rule-based patterns (e.g., "if context_overflow, reduce context slice") with human-confirmed confidence levels. `/wrap` auto-records execution metrics; `/reflect` reviews and confirms patterns.

### Risk-Proportional Evaluation

Replaces the v1 binary light/full evaluator mode. Scales evaluation depth to story risk: **Low** (risk 1–3: automated verification only), **Medium** (risk 4–7: full generator/evaluator), **High** (risk 8+: generator + evaluator + protocol checker). No story ships without some form of review.
