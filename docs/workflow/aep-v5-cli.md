...
`aep eval` reads a project and writes only under the machine-level AEP home (`~/.aep`, or `AEP_HOME`). `aep eval init` writes `~/.aep/config.toml` (eval root, retention, minimum tick interval). `aep eval snapshot` stores structural facts for the project in `~/.aep/eval/<host/owner/repo>/<run>/snapshot.json` with a manifest and checksums; `aep eval report` applies the fixed rules in `aep --skill eval --ref rules` and writes `report.json`/`report.md`; `aep eval record --run <id> --file <json>` stores an observer's finding and its `--note`; `aep eval list` and `aep eval show <run>` read past runs. Under Herdr (`HERDR_ENV=1`), the person opens a pane for the observer, starts an agent there and asks it to watch another agent; `aep eval watch` lists the agents working in the project (own pane excluded) so the agent can confirm the target with the person, and `aep eval watch --target <pane>` prepares the run, saves the parameters as `procedure.md`, takes the baseline snapshot and hands the procedure back to the calling agent, which then observes. Nothing is spawned. `aep eval tick --run <id>` paces the observer: it waits until the configured minimum interval since the previous tick has passed (`min_interval_minutes`, default 10), waits for the target agent to settle, snapshots, and reports only the facts that changed; when nothing changed no snapshot is written and the observer records nothing. The observer's scope is engineering process and AEP adherence; findings cite snapshot facts or transcript evidence and never evaluate product decisions.
