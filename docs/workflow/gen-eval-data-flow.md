...
**Purpose:** Calibration document that tells the evaluator which dimensions to score, how to weight them, and where the hard failure thresholds are. Written once during `/aep-launch`, read-only during the eval loop.

**File:** `.dev-workflow/evaluator-criteria.md`

**Source:** Generated by `/aep-launch` during evaluator brainstorming, based on the presets in `.claude/skills/aep-gen-eval/references/scoring-framework.md`.
...
| Who                                 | Action                    | When                                  |
| ----------------------------------- | ------------------------- | ------------------------------------- |
| `/aep-launch` (main session + user) | Creates the file          | During evaluator brainstorming        |
...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `/aep-launch`    | `evaluator-criteria.md` created (brainstormed with user)                                                                                |
...
## Relationship to /aep-validate

The `/aep-validate` skill uses the same gen/eval pattern but with a different execution model:

| Aspect    | `/aep-build` Phase 5                       | `/aep-validate`                                    |
| --------- | ------------------------------------------ | -------------------------------------------------- |
