...
This repository authors AEP. `crates/` owns the Rust CLI; `skills/native/` supplies embedded instructions; the other skill directories supply the optional legacy v4.1 plugin. Existing `apps/` and `packages/` code is outside the current native CLI deliverable; dashboard work is deferred. Source changes use design documents and implementation branches; do not create a synthetic downstream story loop for repository maintenance.
