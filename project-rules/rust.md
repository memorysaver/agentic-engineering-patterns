...
Native CI targets Linux x86_64 and macOS Apple Silicon (aarch64); macOS Intel is not built. Keep large/binary evidence outside managed text stores and link it with digest and provenance. The CLI is not an OS sandbox or an identity/authorization service. Project check configuration is executable project policy.
