...
For native preview, the accepted `docs/decisions/aep-v5-context-and-self-verification.md` and `aep-v5-preview-adoption.md` supersede legacy F1–F3 fixed evaluator topology and R3 slash-command routing. Use `aep --skill <name>` for native cross-skill discovery. Preserve legacy F1–F3/R3 in the v4 corpus. Shared principles still apply: concise progressive disclosure, canonical resources, observable results, source attribution, and honest capability limits. Native guidance may select same-agent execution or independent review according to the task and explicit project policy.
