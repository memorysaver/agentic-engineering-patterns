...
Set up the tracking infrastructure and environment. The branch already exists (`/aep-launch` created `feat/<name>`). Run these steps in order; each ends in a checkable postcondition.

**Step 0 — Worktree guard, before anything else.** Phases 4–12 run inside this story's feature worktree, verified here rather than assumed. On Codex `codex-subagent` the worktree binding is a soft prompt-contract (`spawn_agent` has no cwd parameter), so a worker can start in the main checkout and create `feat/<name>` there, which breaks Phase 12 autopilot detection. This guard closes that:
...
if [[ "$TOP" != *"/.feature-workspaces/$WS" || "$BRANCH" != "feat/$WS" ]]; then
  ...
  # /aep-launch OWNS worktree creation; the guard only ENTERS the worktree it
  # already made (cd, never `git switch` / `git worktree add -b`).
  ...
  if [ -n "$ROOT" ] && [ -d "$ROOT/.feature-workspaces/$WS" ]; then
    ...
    # No worktree for $WS: escalate (Human-Gate Protocol / needs-human.md)
    # rather than improvising one — "run /aep-launch first, or it misfired."
...
Worktree creation belongs to `/aep-launch`; a missing one is an **escalation**, never improvised in the main checkout, because that is unrecoverable rather than merely wrong. **Postcondition:** guard exits 0 (`show-toplevel` ends in `.feature-workspaces/<name>`, branch is `feat/<name>`).
...
6. **Run project setup.** If `.claude/hooks/workspace-setup.sh` exists, `bash` it (handles deps, dev server, ports, DB/seed, `.env`); otherwise read the README or ask the user how to set up. The hook writes `.dev-workflow/ports.env` with at minimum:
...
8. **Generate the feature verification list** `.dev-workflow/feature-verification.json` (skeleton + generator-ownership rule in `references/harness-artifacts.md`). The generator writes only `commit_sha` — `verification_steps` and `passes` are the evaluator's. **Post:** file exists.
...
**Phase 5 entry — binding tier derivation, before any evaluation round spends the cap.** Re-run the verification-tier derivation from the **actual diff** (`git diff --name-only "$BASE"...HEAD`) against `policy.md`'s `sensitive_paths` and the referee-asset rule, and emit `.dev-workflow/verification-recipe.json` — tier, dimension preset, hard floors, `scope_drift`/`tier_escalated`. Derivation rules, floors, and the recipe schema: `/aep-gen-eval` references/verification-economics.md → The Two-Point Derivation (the generated e2e skill ships a runnable script). Two rules bind here: the tier may only go **up** from the dispatch brief's provisional tier, and a diff outside the declared `files_affected` recomputes from the diff with `scope_drift: true`. **Phase 5 refuses to start without the recipe file** — with a dispatch brief, derive it now; a standalone build with no brief runs `deep` defaults (fail-open) and records a recipe saying so. Publish `verification_tier` and `tier_escalated` to `status.json` (`/aep-launch` references/signals-spec.md owns the schema).
...
4. If a task is incomplete, add a follow-up commit (`feat(<scope>): complete <task>` / `fix(<scope>): <issue>`) and loop back to Phase 4 — the task history stays append-only.

**Quality review — with separate evaluator (full mode):** whether an evaluator runs is decided by the **binding tier in `verification-recipe.json`** (`standard`/`deep` ⇒ evaluator; `light` ⇒ structured self-review) — the criteria file's presence decides nothing. `/aep-launch` writes no `.dev-workflow/evaluator-criteria.md` for a provisional-`light` story, so when the binding derivation upgrades the tier, assemble the criteria **now** from the binding recipe (`dimension_preset` + `hard_floors`; /aep-gen-eval references/scoring-framework.md → How to use presets) before entering the loop — a missing launch-time file must not downgrade the gate. Under a binding `light` tier, run the structured self-review instead and publish its durable completion signal to `status.json` (`self_review: {result, sha, report}` — signals-spec is the schema owner; this is what the autopilot quality gate reads).

With the criteria in place, the evaluator runs via `executor.spawn_evaluator()`, **mode-dispatched** and **worktree-bound** against this workspace's files + git state; per-mode spawn recipes live in `/aep-executor` references/backends.md. **Spawn authority leaves the generator where an orchestrating layer exists** (autopilot nudges the spawn; the main session owns it in interactive runs) — the orchestrator assembles the evaluator's context so the generator cannot curate what its judge sees; only in genuinely standalone builds does the generator invoke the spawn recipe itself. In every mode the evaluator prompt marks `eval-request.md` as the **generator's untrusted claim** — data to verify, never framing to adopt (/aep-gen-eval references/agent-contracts.md).

**Evaluation loop** — for each round N (start 1, **max 2 for every tier that spawns an evaluator**: `light` 0 / `standard` 2 / `deep` 2; a recipe declaring more is still capped at 2 here). Round 2 is bought only by a `blocking` finding.
...
6. **On FAIL, run the taxonomy step first** — every FAIL finding carries an evaluator-authored `Failure-Class` (/aep-gen-eval references/verification-economics.md → Failure Taxonomy) and routes by it: `environment` → ops checklist via the Human-Gate Protocol, zero further rounds; `harness-flake` → quarantine + harness story; `scope` → human gate → `/aep-reflect` re-slicing; unbuilt in-repo dependency → `/aep-dispatch` re-ordering. Only `product-defect` findings continue, and only at `Impact: blocking` — the verdict is derived from the impact labels (eval-protocol.md). A response with no `blocking` finding reads as **PASS** or **PASS with notes**: fix any `material` findings and attest each fix with its evidence (commit, test, or probe) in an addendum to `eval-request.md`, carry `polish` notes into the PR body, and proceed to Phase 6 spending zero further rounds. For a `blocking` finding after round 1: **re-ground** (re-read the spec + design + contracts from scratch), fix, add follow-up commits, then loop to step 1 as round 2 — the PR sees fixes as new commits, so history stays append-only.
...
**Cap exhaustion is defined, not silent:** a `blocking` finding still open after round 2 escalates via the Human-Gate Protocol with type `eval_not_converging`, whatever the tier — there is no `standard → deep` escalation (leave `tier_escalated: false`). The gate record carries the two-round history and **proposes** the strategy changes the human or the autopilot policy may pick: a fresh generator told "the previous approach failed on X; take a different design path" (inheriting this worktree), or decomposing the story to its smallest viable slice with the split surfaced.

**Recovery ladder (generator-side, two rungs)** — round 1: the same generator fixes the `blocking` findings in place; before round 2: **re-ground**, then fix. The evaluator only scores, so re-ground and both gate options stay generator-side. The ladder is `product-defect` machinery only — step 6's taxonomy routes every other class off it before any rung is chosen. **Skip the ladder and escalate immediately** on a hard-failure / security FAIL (auth-model gap, data-exposure risk), a spec contradiction (`scope`), or a missing external dependency (`environment` — needs a named refusal tag): these need human judgment. Rationale + the fresh-generator spawn contract: `/aep-gen-eval` references/recovery-ladder.md.

**Quality review — without evaluator (light mode):** structured self-review across correctness, security, performance, and conventions, walking the `feature-verification.json` steps manually. Document findings in `.dev-workflow/code-review-<feature>.md` and fix issues found.

Update the Phase 5 checkbox. **Signal:** set `status.json` `"phase": 5, "phase_name": "code-review"`.

### The Human-Gate Protocol (any phase)

When you hit a decision **only the human can make** — design ambiguity the spec doesn't resolve, eval non-convergence after max rounds, a product judgment call — raise a gate:
...
2. **Raise it on your launch mode's transport** (`/aep-executor` references/backends.md → The Human-Gate Protocol). The answer comes back through the **main agent** (hub-and-spoke), so the human answers where they already are. In the no-push modes (native-bg-subagent, claude-bg, codex-exec, workflow, headless) that means **gate-and-park**: commit WIP, update `status.json`, and end your run cleanly — your state is in the worktree + `.dev-workflow/`, and the orchestrator resumes a worker there with the answer.
...
- The journey **must map every acceptance criterion to a scenario `Verify`** (or a Tier-1 scripted case for deterministic behavior, or a Tier-3 API check for backend/async state) **before execution**; a deliberately deferred criterion carries a `WAIVER: <reason>` line. That coverage is what `/aep-wrap` checks before flipping the gate to `passed`.
- **Commit the journey file with the feature now (pre-merge)** — even under `journey_timing: post-deploy`, where only the journey's _execution_ defers (Step B).
...
Tier applicability is per project type (a CLI/library layer is `[1,2]`; only a genuine `none`-target layer is all Tier-1, no journey): `/aep-e2e-skill-scaffolding` references/three-tier-model.md.

### Step B — Execute the authored journey (dogfood)

> **Light mode:** skip Step B (Step A still ran). Outside Light mode, Step B runs with whatever host-aware method is available — an absent `agent-browser` means degrade to the next method, not skip.
...
An unmet **required** precondition is a named refusal (`REFUSING [dogfood-secret-absent:<NAME>]`) — surface it via the Human-Gate Protocol as an ops checklist and stop the journey: zero scenarios run, zero rounds spent, `environment` by construction, filed as ops work rather than a code story. An absent **optional** capability (the live half of a non-milestone gate under `live_policy: milestone_gates_only`) is **SKIP**, not REFUSED — it blocks nothing.

**Run the layer's journey** (intent, not a click script), verifying state per each `Verify` line. The journey's `target:` (web/mobile/desktop/cli) plus `skills/e2e-test/tool-selection.md` — the project-local projection of `e2e_tool(target_type)` — resolve which automation tool to use; the host-aware selection and its degradation order are in `/aep-executor` references/dogfood-validation.md (`e2e_tool(target_type)`, web wrapper `dogfood_method()`).

**Resolve the target from `skills/e2e-test/policy.md`** (`dogfood_target`):
...
Whatever the method, emit the unified severity/category/repro report format (dogfood-validation.md → Unified report format) so the classifier stays host-agnostic, and write it to `.dev-workflow/dogfood-<feature>.md` rather than to chat — that path is the ingestion contract `/aep-watch`'s `dogfood_report` source reads to auto-file findings.
...
- **Refine the existing journey** for this layer — fold in selector/route drift, extra `Verify` lines, corrected preconditions. Under `journey_timing: post-deploy` the journey executes later at the `/aep-wrap` gate, so this pre-merge finalize folds in only what local / Tier-3 checks revealed.
- Each `Then` keeps a concrete **Verify** line (API response / state check). Keep it tool-agnostic; API-level assertions use `$BASE_URL` / `$SERVER_URL` from `.dev-workflow/ports.env` (never hardcoded ports).

**Record the layer gate.** Write evidence to `docs/layer-gates/<layer>.md` (the generated `e2e-test` skill ships a `layer-gate-evidence` template): the two coverage matrices — **acceptance traceability** (criterion → proving test → Verify → PASS/FAIL) and **scripted-coverage** (case → asserts → PASS/FAIL) — plus the manual checklist, screenshots / API JSON, and any `WAIVER:` lines. Update `layer_gates[N].coverage` and `evidence.{scripted,journeys,matrix}` in `product-context.yaml`. The two-phase flip (`scripted_passed` → `passed`) happens at `/aep-wrap` (references/layer-advance.md).
...
2. **Review the commit history:** `git log --oneline "$BASE"..HEAD` (resolve `$BASE` per `/aep-git-ref` "Resolving `$BASE`"). It should be a clean linear sequence — one commit per `tasks.md` task, optionally followed by review-fix commits.
3. **Rebase onto the latest integration branch and push** (`-u` on first push): `git fetch origin && git rebase origin/"$BASE"`, resolve conflicts, then `git push -u origin feat/<name>`. Full recipe, including the abort path: `/aep-git-ref` "Publishing & PR Conventions".

---

## Phase 10: Create PR / MR

Open the PR/MR with the base **passed explicitly** — `gh pr create --base "$BASE"` (GitHub) / `glab mr create --target-branch "$BASE"` (GitLab), resolving `$BASE` per `/aep-git-ref` "Resolving `$BASE`"; host auto-detect and the full recipe are in `/aep-git-ref` "Publishing & PR Conventions" → "Open the PR". An unrecognized host means opening it by hand, still with the base passed.

The base is explicit because `gh pr create` otherwise infers it from local branch state and can land the PR on a stale base or — in two-branch mode — on production `main`. **Postcondition:** `gh pr view --json baseRefName` returns `$BASE`.
...
1. **Up-to-date with the integration branch** (resolve `$BASE` per `/aep-git-ref` "Resolving `$BASE`"): `git fetch origin && git rebase origin/"$BASE" && git push --force-with-lease origin feat/<name>` — the lease variant pushes only if the remote hasn't advanced since your last fetch (`/aep-git-ref` "Publishing & PR Conventions").
...
7. **Merge decision.** The `mode` marker is the **sole authority** for detection: the Phase 0 guard relocates every build, interactive included, into a worktree, so cwd cannot distinguish autonomous from interactive.
   ...
   - `mode` reads exactly `autopilot` → **autopilot mode**: **merge immediately** when the pre-merge conditions pass (the orchestrator monitors via signals; nothing is waiting to confirm).
   - anything else (absent, empty, other) → **interactive mode**: ask for confirmation before merging. An ambiguous signal reads as interactive.

   **"PR ready" is not a stop condition.** In autopilot, stop short of merge **only** on the 6 legitimate conditions — those, the full per-state handling, and the worked cases are canonical in `references/merge-decision-cases.md`; anything else merges. **After merging, set `status.json` `story_status: "completed"` — your job ends there.** Wrap runs `/opsx:archive` on the integration branch, so running `/aep-wrap` from this worktree corrupts the concurrency protocol; the orchestrator wraps.
...
- **Only the main session writes `product-context.yaml`** — workspaces report status through `.dev-workflow/signals/status.json` (the concurrency protocol; `/aep-launch` references/signals-spec.md).
