...
Use `aep deliver plan --story <id>` to inspect the exact head and unmet requirements. Eligibility means the recorded verification requirements are satisfied for that candidate. Delivery actions and closure still need observed results. Bring verification and required review current before recording integration.
