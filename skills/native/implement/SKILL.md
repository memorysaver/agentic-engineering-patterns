...
Exercise changed behavior during implementation using the project's verification procedure. Read `aep --skill validate` to establish completion evidence. Carry the actual commit, performed checks, findings, remaining gaps and requested delivery endpoint into `aep --skill deliver`. Continue authorized delivery and closure after validation. A worker returning a verified candidate hands responsibility back to the coordinating agent, which continues toward the user’s outcome.
