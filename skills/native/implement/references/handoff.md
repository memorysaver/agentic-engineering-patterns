...
On recovery, compare the attempt and worktree with actual commits, dirty files, processes and host state. Continue valid work and checks, reestablish missing setup, and explain contradictory facts before making another claim. Preserve unrelated work. Record progress near canonical task records or in the handoff instead of maintaining competing status databases.

If a selected native capability is unavailable, identify the gap and remaining work. Explicitly change workflow ownership before using another workflow to write state. A legacy source document can inform context without reactivating its old writers.
