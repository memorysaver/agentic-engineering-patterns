...
Configure repeatable scripted probes in `.aep/config.toml` using the current `aep config` contract. Register every gate the project's CI or deployment pipeline enforces on a candidate (manifest, admission, policy or secret checks) as a configured check with the same command, so `aep verify run` covers what the pipeline will reject. Secret scanners commonly flag native record digests (`fingerprint`, `binary_sha256`, `guidance_digest`, revision hashes); scope any allowlist by store path and field rather than by value. Interactive observations can remain durable notes/artifacts with revision and environment; describe that evidence boundary accurately. AEP executes configured checks, rather than providing a universal app driver.
