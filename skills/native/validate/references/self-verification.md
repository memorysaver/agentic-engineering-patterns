...
`verify run` executes configured commands and records their result/environment/head/fingerprint and the executing AEP version/binary digest. Review requests, responses and delivery/publication records also retain their producer identity. Keep a trial on an immutable executable path when possible; record any version cutover rather than reporting only the starting binary. A successful process exit establishes what that command actually checked. Arbitrary screenshots, prototype files and linked external documents are not automatically digested or assessed for acceptance coverage by the gate. Preserve their identity and location in durable notes/artifacts, and state the manual coverage/enforcement boundary.
