...
(post-deploy). `e2e_tool(target_type)` covers **web / mobile / desktop / cli**
targets; `dogfood_method()` is the `:= e2e_tool('web')` wrapper.
...
5. [Post-deploy worker & boundary](#post-deploy-worker--boundary)
...
`executor.detect()` resolves HOST + mode; this adds a tool probe on top,
...
`dogfood_method()` is the thin wrapper (`:= e2e_tool('web')`) that
`/aep-build` Phase 6 and the autopilot post-merge guard call.
...
e2e_tool(target_type):              # target_type ∈ {web, mobile, desktop, cli}; default web (aep-vocab: journey_target_type)
  ...
  if target_type == web and (pref unset or pref == 'auto'):
    pref = topology.routing.dogfood.method                  # deprecated alias of e2e.tool.web
...
dogfood_method() := e2e_tool('web')   # the wrapper Phase 6 / post-merge-guard call
```
...
- **`env=local`** — source `.dev-workflow/ports.env`, return `$BASE_URL`
  (`ports.env` is written by the workspace-setup hook).
...
- **`method`** — deprecated alias of `e2e.tool.web`; `auto` defers to
