...
derived dimension preset + hard floors) into the machine-assembled brief. Run the derivation
function over the story's **plan fields** — declared `files_affected`, `module`, contract
obligations, `complexity`, `on_critical_path`, layer kind — plus `policy.md`'s `sensitive_paths`;
the function, its tier rules, and the recipe mapping are canonical in `/aep-gen-eval` →
`references/verification-economics.md` → The Two-Point Derivation.
