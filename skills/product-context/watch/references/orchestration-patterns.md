...
Coverage here is **acceptance/requirements coverage**, not a line/branch percentage — the question is "is every behavior this layer promised actually proven?", not "what fraction of lines ran". `/aep-build` Phase 6 is **journey-first**: Step A **authors the journey FILE pre-merge** from the layer's `acceptance_criteria` (one scenario per criterion, before any dogfood), committed with the feature — independent of `journey_timing`, which governs only **when** the journey is _executed_ (pre-merge, or at the post-deploy `/aep-wrap` gate). Any criterion still uncovered at build is authored then, never deferred. A layer reaching its gate with no authored journey is a COVERAGE FAILURE — `/aep-wrap` **executes, never authors**. Advancing to the _next_ layer is then a human-confirmed step (`/aep-wrap`), not automatic.
