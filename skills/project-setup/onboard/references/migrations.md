...
6. `skills/e2e-test/scripts/derive-verification-recipe.sh` is **rendered from an AEP template**, so the re-pin alone leaves it at the old caps — and `/aep-e2e-skill-scaffolding`'s upgrade mode writes only files that are absent, so it will not re-render an existing script either. Patch it by hand: in the `case "$TIER"` block set `standard` and `deep` to `MAX_ROUNDS=2; EFFORT="high"`, and change the negative-assertion-delta line's `EFFORT="default"` to `EFFORT="high"`. (The alternative — delete the script and re-run `/aep-e2e-skill-scaffolding`, which then re-renders the absent file — works but touches nothing else, so the hand patch is shorter.) Verify with `bash skills/e2e-test/scripts/derive-verification-recipe.sh` on any story: the emitted recipe says `"max_rounds": 2` and `"evaluator_effort": "high"`. Existing `.dev-workflow/verification-recipe.json` files need no edit — the protocol's cap governs and the old effort values are read as `high`.
