...
0. [Quick Reference: project types, presets, options, constraints](#quick-reference-project-types-presets-options-constraints)
...
## Quick Reference: project types, presets, options, constraints

Consult these tables when building the `create-better-t-stack` command (`/aep-scaffold` Phase 1/Phase 3).

### Project type → recommended preset

| Project type                 | Recommended preset                            |
| ---------------------------- | --------------------------------------------- |
| **SaaS / web app**           | Default stack (see Recommended Combinations)  |
...
If the user's project matches a well-known pattern, the CLI has `--template` presets that skip all
selection. Only suggest a template if it matches the user's needs exactly.
...
### All available options (flags + defaults)
